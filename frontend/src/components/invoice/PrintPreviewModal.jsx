...
import A5InvoiceTemplate from './A5InvoiceTemplate';
import Thermal80InvoiceTemplate from './Thermal80InvoiceTemplate';
import { useSettings } from '../../context/SettingsContext';

/**
 * Print Preview Modal
 * Shows A4, A5, or 80mm thermal preview and triggers window.print().
...
export default function PrintPreviewModal({ invoice, isOpen, onClose, initialFormat }) {
  ...
  // Margin 6mm: @page handles the page margin; the .a5-invoice div uses padding:0
  // so the full printable width (148mm - 2×6mm = 136mm) is used for content.
  const A5_PRINT_CSS = `
    ...
    try {
      ...
      if (printWindow) {
        printWindow.document.write(`
          ...
          <html>
            <head>
              <title>${invoice.invoice_number} — ${isA5 ? 'A5 Invoice' : is80mm ? '80mm Receipt' : 'A4 Invoice'}</title>
        ...
        <div className="modal-body" style={{ background: '#f0f0f0', padding: '16px 12px', overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ overflowX: 'auto', width: '100%', WebkitOverflowScrolling: 'touch' }}>
            <div ref={printRef} style={{ display: 'flex', justifyContent: 'center', minWidth: previewMinWidth, margin: '0 auto' }}>
              {format === 'A4' && <A4InvoiceTemplate invoice={invoice} shopSettings={settings} />}
              {format === 'A5' && <A5InvoiceTemplate invoice={invoice} shopSettings={settings} />}
              {format === '80mm' && <Thermal80InvoiceTemplate invoice={invoice} shopSettings={settings} />}
            </div>
          </div>
        </div>
      </div>
